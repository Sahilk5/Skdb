...
template <class Stoppoint>
typename stoppoint_collection<Stoppoint>::points_t::iterator
stoppoint_collection<Stoppoint>::find_by_id(typename Stoppoint::id_type id) {
    return std::find_if(begin(stoppoints_), end(stoppoints_),
        [=](auto& point) { return point->id() == id; });
}

template <class Stoppoint>
typename stoppoint_collection<Stoppoint>::points_t::const_iterator
stoppoint_collection<Stoppoint>::find_by_id(typename Stoppoint::id_type id) const {
    return const_cast<stoppoint_collection*>(this)->find_by_id(id);
}

template <class Stoppoint>
typename stoppoint_collection<Stoppoint>::points_t::iterator
stoppoint_collection<Stoppoint>::find_by_address(virt_addr address) {
    return std::find_if(begin(stoppoints_), end(stoppoints_),
...
template <class Stoppoint>
typename stoppoint_collection<Stoppoint>::points_t::const_iterator
stoppoint_collection<Stoppoint>::find_by_address(virt_addr address) const {
