...
void sdb::process::resume() {
	...
	if (ptrace(PTRACE_CONT, pid_, nullptr, nullptr) < 0) {
		error::send_errno("Couldn't continue");
	}
...
void sdb::process::read_all_registers() {
	...
	if (ptrace(PTRACE_GETFPREGS, pid_, nullptr, &get_registers().data_.i387) < 0) {
		error::send_errno("Could no read FPR registers");
	}
...
std::vector<std::byte> sdb::process::read_memory(
	...
	if (process_vm_readv(pid_, &local_desc, /*liovcnt=*/1,
				remote_descs.data(),  /*reiovcnt=*/remote_descs.size(), /*flags=*/0) < 0) {
