...
TEST_CASE("process::launch no such program", "[process]") {
	REQUIRE_THROWS_AS(process::launch("wowowwoowow"), error);
}
...
TEST_CASE("process::resume alread terminated", "[process]") {
...
TEST_CASE("Can find breakpoint site") {
	auto proc = process::launch("targets/run_endlessly");
...
TEST_CASE("Cannot find breakpoint site") {
	auto proc = process::launch("targets/run_endlessly");
	const auto* cproc = proc.get();
...
TEST_CASE("Breakpoint site list size and emptiness") {
	auto proc = process::launch("targets/run_endlessly");
	const auto* cproc = proc.get();
...
TEST_CASE("Can iterate breakpoint sites") {
	auto proc = process::launch("targets/run_endlessly");
	const auto* cproc = proc.get();
